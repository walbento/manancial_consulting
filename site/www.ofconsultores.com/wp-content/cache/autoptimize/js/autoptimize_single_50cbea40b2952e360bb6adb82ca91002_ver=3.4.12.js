...
var target=getEventTargetDeep(event);if(event.defaultPrevented||event.ctrlKey){return true;}
...
if((isNodeName(target,'button')||isNodeName(target,'input')&&buttonTypes.test(target.type))&&event.code==='Space'){return true;}
if(isNodeName(target,'input')&&target.type=='radio'&&(event.code==='ArrowUp'||event.code==='ArrowDown'||event.code==='ArrowLeft'||event.code==='ArrowRight')){return true;}
...
function overflowingAncestor(el){var elems=[];var body=document.body;var rootScrollHeight=root.scrollHeight;while(el){var cached=getCache(el,false);if(cached){return setCache(elems,cached);}
...
function isInsideYoutubeVideo(event){var elem=getEventTargetDeep(event);var isControl=false;if(document.URL.indexOf('www.youtube.com/watch')!=-1){do{isControl=(elem.classList&&elem.classList.contains('html5-video-controls'));if(isControl)break;}while((elem=elem.parentNode));}
return isControl;}
